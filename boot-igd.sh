/home/max/Code/qemu/x86_64-softmmu/qemu-system-x86_64 \
    -enable-kvm -m 10000 -cpu Penryn,kvm=on,vendor=GenuineIntel,+invtsc,vmware-cpuid-freq=on \
    -bios /home/max/Code/seabios/out/bios.bin \
	  -machine pc-i440fx-4.0   \
	  -smp 4,cores=2 \
	  -device isa-applesmc,osk="ourhardworkbythesewordsguardedpleasedontsteal(c)AppleComputerInc" \
	  -smbios type=2 \
    -device vfio-pci,host=00:02.0,bus=pci.0,addr=0x2,x-igd-opregion=on,x-vga=on,romfile=/vbios.dump \
	  -device ahci,id=ahci \
    -drive id=disk0,file=/dev/disk/by-id/ata-KINGSTON_SA400S37240G_50026B77826366B7,format=raw,if=none \
    -device ide-drive,drive=disk0,bus=ahci.0 \
	  -netdev user,id=net0 -device e1000-82545em,netdev=net0,id=net0,mac=52:54:00:c9:18:27 \
    -chardev file,id=seabios,path=/tmp/bios.log \
    -device isa-debugcon,iobase=0x402,chardev=seabios \
    -vga none \
    -vnc 0.0.0.0:1,password=off \
    -nographic
